 ...
 * the License.
 */
package info.joyc.generator.config.builder;

import info.joyc.generator.annotation.DbType;
import info.joyc.generator.InjectionConfig;
import info.joyc.generator.config.*;
import info.joyc.generator.config.po.TableField;
import info.joyc.generator.config.po.TableFill;
...
public class ConfigBuilder {
    ...
    private void checkImportPackages(TableInfo tableInfo) {
        ...
        //    // 无父类开启 AR 模式
        //    tableInfo.getImportPackages().add(info.joyc.extension.activerecord.Model.class.getCanonicalName());
        }
        if (null != globalConfig.getIdType()) {
            // 指定需要 IdType 场景
            tableInfo.getImportPackages().add(info.joyc.generator.annotation.IdType.class.getCanonicalName());
            tableInfo.getImportPackages().add(info.joyc.generator.annotation.TableId.class.getCanonicalName());
        }
        if (StringUtils.isNotEmpty(strategyConfig.getVersionFieldName())) {
            tableInfo.getFields().forEach(f -> {
                if (strategyConfig.getVersionFieldName().equals(f.getName())) {
                    tableInfo.getImportPackages().add(info.joyc.generator.annotation.Version.class.getCanonicalName());
                }
            });
        }
    }
