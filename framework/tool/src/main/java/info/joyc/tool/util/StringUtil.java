...
import info.joyc.tool.comparator.VersionComparator;
import info.joyc.tool.lang.Assert;
import info.joyc.tool.lang.Matcher;
import info.joyc.tool.text.StrFormatter;
import info.joyc.tool.text.StrSpliter;
 ...
 * @see cn.hutool.core.util.StringUtil
 *
 * @author : qiuzh
 * @version : v1.0.0
 * @desc : TODO
 * @since : 2018-11-29 15:37
 */
public class StringUtil {
     ...
     * @return 替换后的字符串
     */
    public static String replaceAndFirstToUpper(String srcStr, String oldMark, String newMark) {
        StringBuilder newString = new StringBuilder();
        int first = 0;
        while (srcStr.contains(oldMark)) {
            first = srcStr.indexOf(oldMark);
            if (first != srcStr.length()) {
                newString.append(srcStr, 0, first).append(newMark);
                srcStr = srcStr.substring(first + oldMark.length(), srcStr.length());
                srcStr = toUpperCaseFirstOne(srcStr);
            }
        }
        newString.append(srcStr);
        return toUpperCaseFirstOne(newString.toString());
    }

    /**
     * 为枚举服务：将code中_替换掉，并开头字母转大写
     *
     * @param srcStr
     * @return
     */
    public static String convertForEnum(String srcStr) {
        return replaceAndFirstToUpper(srcStr, "_", "");
    }

    /**
     * 传入List集合，返回以指定符号分隔及单引号包含的字符串
     *
     * @param strList   需要分隔的集合
     * @param separator 分隔符
     * @return 字符串
     */
    public static String separateProcessing(Iterable<String> strList, String separator) {
        ...
        if (strList != null) {
            ...
            while (iterator.hasNext()) {
                if (StringUtil.isNotEmpty(iterator.next())) {
                    sb.append("'").append(iterator.next()).append("'").append(separator);
                }
            }
            String result = sb.toString();
            String tp = result.substring(result.length() - 1, result.length());
            if (separator.equals(tp)) {
    ...
    /**
     * 传入集合，返回以指定符号分隔的字符串，且若为连号则使用中杠线连接
     *
     * @param strCollection 需要分隔且连接的集合
     * @param separator     分隔符
     * @param joiner        连接符
     * @return 字符串
     */
    public static String separateJoin(Collection<String> strCollection, String separator, String joiner) {
        if (StringUtil.isEmpty(separator)) {
            separator = ",";
        }
        if (strCollection != null) {
            TreeSet<String> source = new TreeSet<>(strCollection);
            ...
            while (iterator.hasNext()) {
                ...
                if (StringUtil.isNotEmpty(current)) {
                    if (StringUtil.isNotBlank(previous) && Integer.parseInt(current) == Integer.parseInt(previous) + 1) {
                        isNext = true;
                    } else {
                        if (isNext) {
                            sb.append(joiner).append(previous);
                            isNext = false;
                        }
                        sb.append(separator).append(current);
                    }
                    if (isNext && !iterator.hasNext()) {
                        sb.append(joiner).append(current);
                        isNext = false;
                    } else {
                        previous = current;
                    }
            ...
            if (sb.length() > 0) {
                String result = sb.toString();
                String tp = result.substring(0, 1);
                if (separator.equals(tp)) {
